...
All commits must follow the Conventional Commits specification for clear, searchable git history.
...
- No trailing period
- Must contain task ID followed by brief description

**Body**:

- First paragraph: Business language describing why changes were made
- Blank line
- Second paragraph: Technical details in bullet points
- Blank line
...
```
feat(backlog): Add sprint planning template

Implement standardized sprint planning document structure for consistent sprint documentation.

- Add sprint goal and duration fields
- Include user story breakdown table
- Add task assignment and tracking columns
- Include Definition of Done checklist
- Link to feature request documents
...
1. **Before Work Begins**: Create feature branch from `main`
2. **During Work**: Make atomic commits with clear messages
3. **After Work**: Ensure all commits follow conventional format
4. **Pull Request**: Squash commits if needed, ensure PR title follows convention

### Linking Commits to Backlog Items

All commits must reference the relevant backlog item:

- Feature requests: `Refs: FR-001`
- Bug fixes: `Refs: BF-001`
- Sprints: `Refs: Sprint-01`

**Example Commit Links**:

```
feat(llm): Implement Ollama provider integration
